...
impl Check<TapReceipt> for PayerCheck {
    async fn check(
        ...
        let receipt_payer = receipt.signed_receipt().payer();
        ...
        if receipt_payer == *recovered_sender {
            Ok(())
        } else {
            Err(CheckError::Failed(anyhow::anyhow!(
                "Invalid payer: receipt claims payer {receipt_payer} but signer is authorized for {recovered_sender}",
            )))
        }
    }
}
