...
impl Header for TapHeader {
        ...
        I: Iterator<Item = &'i HeaderValue>,
    {
        let mut execute = || -> anyhow::Result<TapHeader> {
            ...
            let raw_receipt = BASE64_STANDARD.decode(raw_receipt).map_err(|e| {
                tracing::debug!(error = %e, "Failed to base64 decode v2 receipt");
                e
            })?;
            tracing::debug!(
            ...
            let receipt = grpc::v2::SignedReceipt::decode(raw_receipt.as_ref()).map_err(|e| {
...
mod test {
    ...
    // V1 receipt decoding test removed - V1 support has been dropped
